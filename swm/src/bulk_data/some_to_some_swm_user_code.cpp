...
SomeToSomeSWMUserCode::SomeToSomeSWMUserCode(
...
void
SomeToSomeSWMUserCode::call()
{

    //std::cout << std::endl << "process_id: " << process_id << " Starting up";
  ...
  //SWMPiggybackBase* dummy_piggyback = nullptr;
    ...
    if (process_id >= min_src_id && process_id <= max_src_id) // senders only
    {
